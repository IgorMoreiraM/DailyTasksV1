...
export function ChangePasswordPage() {
  const { isAuthenticated, senhaTemporaria, role, username, logout } = useAuth()
  ...
  if (!senhaTemporaria && !success) return <Navigate to="/"      replace />

  const tooShort  = nova.length > 0 && nova.length < 6
  const noMatch   = confirma.length > 0 && nova !== confirma
  const canSubmit = nova.length >= 6 && nova === confirma && !loading
  ...
  async function handleSubmit(e: FormEvent) {
    ...
    try {
      // 1. Altera a senha
      await funcionarioApi.alterarSenha(nova)
      setSuccess(true)

      // 2. Faz login novamente para obter token com senhaTemporaria = false
      if (username) {
        try {
          const res = await authApi.login(username, nova)
          // Atualiza o token no localStorage diretamente
          localStorage.setItem('dt_token', res.data.token)
        } catch {
          // Se o re-login falhar, faz logout e manda para o login
          setTimeout(() => {
            logout()
      ...
      // 3. Redireciona pelo papel após pequeno delay
      setTimeout(() => {
    ...
    <div className="min-h-screen flex items-center justify-center p-6" style={{ background: 'var(--bg-page)' }}>
      ...
      >
        <div className="flex flex-col items-center mb-8">
          ...
          <h1 className="font-display font-bold text-[22px] text-center" style={{ color: 'var(--text-primary)' }}>
            Crie sua senha
          </h1>
          <p className="text-[13px] text-center mt-1.5" style={{ color: 'var(--text-secondary)' }}>
            Você está acessando com uma senha temporária.<br />
        ...
        {success ? (
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            ...
            <div className="flex flex-col gap-1.5">
              ...
              <div className="relative">
                ...
                <input
                  ...
                  style={{ background: 'var(--bg-subtle)', borderColor: tooShort ? '#ef4444' : 'var(--border-default)', color: 'var(--text-primary)' }}
                />
              ...
              {nova.length > 0 && (
                <div className="flex items-center gap-2 mt-1">
                  ...
                  <span className="text-[11px] font-semibold" style={{ color: str.color }}>{str.label}</span>
                </div>
              )}
            ...
            <div className="flex flex-col gap-1.5">
              ...
              <div className="relative">
                ...
                <input
                  ...
                  required
                  className="w-full h-11 pl-10 pr-11 rounded-xl border text-[13.5px] outline-none transition-all focus:border-brand-teal focus:shadow-[0_0_0_3px_rgba(42,122,138,0.12)]"
                  style={{ background: 'var(--bg-subtle)', borderColor: noMatch ? '#ef4444' : 'var(--border-default)', color: 'var(--text-primary)' }}
                />
            ...
            <div className="rounded-xl p-3.5 space-y-1.5" style={{ background: 'var(--bg-subtle)' }}>
              ...
              {[
                ...
                <div key={r.text} className="flex items-center gap-2">
                  <div className={`w-3.5 h-3.5 rounded-full flex items-center justify-center ${r.ok ? 'bg-emerald-500' : 'bg-slate-200'}`}>
                    {r.ok && (
                      <svg width="8" height="8" viewBox="0 0 10 10">
                        <path d="M2 5l2.5 2.5 4-4" stroke="white" strokeWidth="1.5" strokeLinecap="round" fill="none" />
                      </svg>
                    )}
                  </div>
                  <span className="text-[12px]" style={{ color: r.ok ? 'var(--text-secondary)' : 'var(--text-muted)' }}>
                    {r.text}
                  </span>
                </div>
              ))}
            ...
            <button
              type="submit"
              disabled={!canSubmit}
              className="w-full h-11 rounded-xl font-semibold text-[14px] text-white transition-all disabled:opacity-50 flex items-center justify-center"
              style={{
                background: canSubmit ? '#f97316' : '#94a3b8',
